...
- сценарный лендинг с переключателем `Я учусь / Я автор`, FAQ и конверсионными CTA
...
Лендинг больше не объясняет внутреннюю кухню платформы. Подача строится вокруг двух пользовательских сценариев:

- ученик выбирает курс под задачу сделки, сразу видит формат, цену, уроки и следующий шаг
- автор понимает, как разместить программу, какие форматы поддерживаются и что останется внутри урока после публикации

На первом экране есть переключатель `Я учусь / Я автор`. Он меняет заголовок, CTA, блок «Как это работает» и FAQ, а выбранный режим сохраняется в URL и `localStorage`.

Каталог продолжает эту же логику: курс показывается как понятный продукт с темой, форматом, количеством уроков, результатом и прозрачным следующим шагом.

Экран входа и checkout тоже остаются в этом же языке: без demo-подсказок и служебных комментариев, с фокусом на понятный пользовательский путь до доступа к курсу.
...
В студии урока блоки теперь открываются в двух режимах: просмотр и отдельное редактирование. Это убирает одновременный показ превью блока и полей ввода в одном состоянии.
