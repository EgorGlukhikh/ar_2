...
export function CourseCoverUploadField({
  ...
  async function handleFileSelection(file: File) {
    ...
    try {
      ...
      setMessage(
        `Файл загружен: ${payload.file.filename} (${formatFileSize(payload.file.sizeInBytes)}).`,
      );
    } catch (uploadError) {
      setError(
      ...
      <div className="space-y-2">
        ...
        <div className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-4">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
            Только файл. Внешние ссылки больше не используются. Рекомендуемое соотношение 16:9.
          </p>

          {filename ? (
            <p className="mt-2 text-sm font-medium text-[var(--foreground)]">{filename}</p>
          ) : null}
            ...
            <div className="h-32 w-full bg-[linear-gradient(135deg,rgba(92,98,255,0.08),rgba(255,179,214,0.1))] sm:h-36">
              {coverUrl ? (
                ...
                <div className="flex h-full items-center justify-center px-6 text-center text-sm leading-6 text-[var(--muted)]">
                  После загрузки здесь появится превью обложки курса.
                </div>
              )}
            </div>
          </div>
